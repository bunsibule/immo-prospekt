...
export default function App() {
  ...
  useEffect(() => {
    const ch = supabase.channel("hunter2").on("postgres_changes",{event:"*",schema:"public",table:"prospects"},loadShortlist).subscribe();
...
Fouille : leboncoin.fr, pap.fr, Facebook Marketplace, groupes Facebook locaux de l'Aude, journaux annonces légales Aude.
...
[{
  ...
  "signal": "signal de motivation vendeur détecté, ex: baisse de prix -15%, succession en cours, mutation pro",
  "note": "1 phrase : pourquoi c'est une bonne piste à appeler"
}]
6 à 10 résultats maximum. Si vraiment rien trouvé : [].`;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 400) setSearchErr("Clé API invalide. Vérifie ta clé Gemini dans les paramètres.");
        else if (res.status === 429) setSearchErr("Trop de requêtes. Attends quelques secondes et réessaie.");
      ...
      setSearchErr("Erreur de connexion. Vérifie ta connexion internet et réessaie.");
    }
    setSearching(false);
  }
    ...
    try {
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            contents:[{ role:"user", parts:[{ text:`Rédige un message de prospection court et chaleureux pour Sophie, mandataire immobilière dans l'Aude.
Prospect : ${p.name}, bien : ${p.type}, zone : ${p.zone}, source : ${p.source}.
Contexte : ${p.note||"aucun"}.
Français, moins de 80 mots, humain et naturel, pas commercial. Proposer un simple appel. Signer "Sophie – Mandataire immobilière, Narbonne & Corbières". Message uniquement, sans introduction.` }] }],
            generationConfig:{ maxOutputTokens:300 }
          })
        }
      );
      const d = await res.json();
      setGenMsg(d.candidates?.[0]?.content?.parts?.[0]?.text || "Erreur génération.");
      ...
      <div style={{ width:220,background:"rgba(255,255,255,.02)",borderRight:"1px solid rgba(255,255,255,.06)",padding:"28px 14px",display:"flex",flexDirection:"column",gap:6,flexShrink:0 }}>
        ...
        <div style={{ marginTop:"auto",paddingTop:14,borderTop:"1px solid rgba(255,255,255,.05)" }}>
          <div style={{ display:"flex",alignItems:"center",gap:7,padding:"6px 8px" }}>
            <span style={{ width:7,height:7,borderRadius:"50%",background:apiKey?"#10b981":"#ef4444",display:"inline-block",flexShrink:0 }}/>
                ...
                <div>
                  <div style={{ display:"flex",alignItems:"center",gap:10,background:"rgba(16,185,129,.08)",border:"1px solid rgba(16,185,129,.2)",borderRadius:10,padding:"12px 16px",marginBottom:16 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:12,color:"#4b5563",marginTop:2 }}>{apiKey.slice(0,8)}••••••••{apiKey.slice(-4)}</div>
                    </div>
                  </div>
                  <button onClick={()=>{ localStorage.removeItem("gemini_key"); setApiKey(""); showToast("Clé supprimée","info"); }} style={{ ...btnX,color:"#ef4444",border:"1px solid rgba(239,68,68,.2)",fontSize:12 }}>
                    🗑 Supprimer la clé
                  </button>
                </div>
              ) : (
                <div>
                  <div style={{ display:"flex",alignItems:"center",gap:10,background:"rgba(239,68,68,.06)",border:"1px solid rgba(239,68,68,.2)",borderRadius:10,padding:"12px 16px",marginBottom:18 }}>
                    <span style={{ fontSize:18 }}>⚠️</span>
                    <div style={{ fontSize:13,color:"#f87171" }}>Aucune clé configurée — la chasse est désactivée</div>
                  </div>
          ...
          <div style={{ animation:"fadeIn .3s ease" }}>
            ...
            {searching && (
              <div style={{ textAlign:"center",padding:"50px 0" }}>
                ...
                <div style={{ fontSize:13,color:"#4b5563" }}>Fouille LBC, PAP, Facebook, annonces légales Aude</div>
                      ...
                      <div key={i} className="rcard" style={{ background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.08)",borderRadius:14,padding:"20px 24px",transition:"all .15s",animation:`fadeIn .3s ease ${i*.05}s both` }}>
                        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:16 }}>
                          ...
                          <div style={{ display:"flex",flexDirection:"column",gap:8,flexShrink:0,alignItems:"flex-end" }}>
                            {r.url
                              ? <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ ...btnG,padding:"9px 18px",fontSize:12,textDecoration:"none",display:"inline-block" }}>Voir l'annonce →</a>
                              : <span style={{ fontSize:11,color:"#374151" }}>Lien non dispo</span>}
                            {already
                              ? <span style={{ fontSize:12,color:"#10b981" }}>✓ Sauvegardé</span>
                              : <button onClick={()=>sauvegarder(r)} style={{ ...btnX,fontSize:12,padding:"7px 14px" }}>⭐ Sauvegarder</button>}
                          </div>
                        </div>
                      </div>
                    );
